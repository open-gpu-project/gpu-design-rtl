...
async def check_bram_rows(dut, lines, *, start_addr, count, name):
   ...
   for offset in range(count + 1):
      ...
      if actual != expected:
         ...
         raise AssertionError(f"{name} BRAM A[{addr}] mismatch\n"
...
def observe_fma_output(dut, *, cycle, pending_high, pending_low, observed_high_latencies,
                       observed_low_latencies, observed_nonzero):
...
def format_lanes(values):
   return (f"l0=(0x{values[0]:05x}, 0x{values[1]:05x}) "
