...
def observe_acc_in_output(dut, *, cycle, pending_preload, pending_add, observed_preload_latencies,
                          observed_add_latencies, observed_nonzero):
   ...
   if pending_preload and actual == (pending_preload[0].expected_l0,
                                     pending_preload[0].expected_l1):
...
def assert_no_pending(dut, pending, observed_nonzero, name):
   ...
   raise AssertionError(f"{name} result not observed\n"
